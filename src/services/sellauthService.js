...
[{"type":"de_faq","content":"..."},{"type":"en_faq","content":"..."},{"type":"price_list","content":"..."}]`;
  ...
  async syncToKnowledgeBase(apiKey, shopId, shopUrl, jobId) {
    let syncJobManager = null;
    try { syncJobManager = require('./syncJobManager'); } catch (_) { syncJobManager = null; }
    const progress = (pct, step) => {
      if (jobId && syncJobManager) syncJobManager.updateProgress(jobId, pct, step);
