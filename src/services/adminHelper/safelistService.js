...
const safelistService = {
  ...
  async generateAiSummary(channelId, username, userId) {
    ...
    // NUR echte bestätigte Feedbacks — KEIN Halluzinieren
    const feedbacks = await this.getFeedbacks(channelId, username, userId);
    const approved  = (feedbacks || []).filter(f => f.status === "approved" && !f.deleted_at);
    if (!approved.length) return null;  // Keine Feedbacks → keine Zusammenfassung

    // Cache prüfen
    let repQ = supabase.from("user_reputation")
      .select("ai_summary, ai_summary_updated_at")
      .eq("channel_id", String(channelId));
    if (userId) repQ = repQ.eq("user_id", userId);
    else if (username) repQ = repQ.ilike("username", username);
    const { data: rep } = await repQ.maybeSingle();

    const newestFb  = new Date(approved[0].created_at).getTime();
    const cacheTime = rep?.ai_summary_updated_at ? new Date(rep.ai_summary_updated_at).getTime() : 0;
    if (rep?.ai_summary && cacheTime >= newestFb) return rep.ai_summary;

    const allText = approved
      .slice(0, 10)
      .map(f => `[${f.feedback_type === "positive" ? "+" : "-"}] ${(f.feedback_text || "").substring(0, 120)}`)
      .join("\n");
    try {
      const resp = await axios.post("https://api.openai.com/v1/chat/completions", {
        model: "gpt-4o-mini", max_tokens: 200, temperature: 0.1,
        messages: [
          { role: "system", content: "Du fasst Handels-Feedbacks präzise zusammen. Nur auf Basis der gegebenen Feedbacks — nichts erfinden. Max 3 kurze Sätze." },
          { role: "user",   content: `Zusammenfassung für @${username || userId} basierend auf ${approved.length} Feedback(s):\n${allText}` }
        ]
      }, { headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, timeout: 15000 });

      const summary = resp.data.choices[0].message.content.trim();
      if (rep) {
        let updQ = supabase.from("user_reputation")
          .update({ ai_summary: summary, ai_summary_updated_at: new Date().toISOString() })
          .eq("channel_id", String(channelId));
        if (userId) updQ = updQ.eq("user_id", userId);
        else updQ = updQ.ilike("username", username || "");
        await updQ;
      }
      return summary;
    } catch { return null; }
  ...
  /** Löscht alte temporäre Bot-Nachrichten aus dem Log (Scheduler-Aufruf). */
  async pruneOldMessageLog(maxAgeMs = 48 * 60 * 60 * 1000) {
    try {
      const cutoff = new Date(Date.now() - maxAgeMs).toISOString();
      await supabase.from("channel_message_log")
        .delete()
        .lt("created_at", cutoff)
        .eq("is_temp", true);
    } catch (e) {
      logger.warn(`[safelistService] pruneOldMessageLog Fehler: ${e.message}`);
    }
  },
  ...
  async submitFeedback({ channelId, submittedBy, submittedByUsername, targetUserId, targetUsername, feedbackType, feedbackText }) {
    const { data, error } = await supabase.from("user_feedbacks").insert([{
      ...
      target_username:        (targetUsername || "").toLowerCase().trim() || null,
      feedback_type:          feedbackType,
      feedback_text:          feedbackText || null,
      submitted_by:           String(submittedBy),
      submitted_by_username:  submittedByUsername || null,
      status:                 "pending"
    }]).select("id").single();

    if (error) {
      logger.warn(`[safelistService] submitFeedback Fehler: ${error.message}`);
  ...
  async approveFeedback(feedbackId, adminUserId, channel) {
    let fb = null;
    try {
      const r2 = await supabase.from("user_feedbacks").select("*").eq("id", feedbackId).single();
      fb = r2.data;
    } catch (_) {}
    if (!fb) {
      logger.warn(`[safelistService] approveFeedback: Feedback ${feedbackId} nicht gefunden`);
      return null;
    }

    const { error: updErr } = await supabase.from("user_feedbacks")
      .update({ status: "approved", reviewed_by: String(adminUserId), updated_at: new Date().toISOString() })
      .eq("id", feedbackId);
    if (updErr) {
      logger.warn(`[safelistService] approveFeedback update Fehler: ${updErr.message}`);
      return null;
    }
    ...
    // Reputation aktualisieren: zuerst RPC, Fallback auf _recalculate
    try {
      ...
      // Fallback: manuell neu berechnen
      await this._recalculateUserReputation(fb.channel_id, fb.target_username, fb.target_user_id);
    }

    // Score für Auto-Scam-Check laden
    let currentScore = 0;
    try {
      let repQ = supabase.from("user_reputation").select("score").eq("channel_id", String(fb.channel_id));
      if (fb.target_user_id) repQ = repQ.eq("user_id", fb.target_user_id);
      else repQ = repQ.ilike("username", fb.target_username || "");
      const repCheck = await repQ.maybeSingle();
      currentScore = repCheck.data?.score || 0;
    } catch (_) {}

    // Auto-Scam bei Score ≤ -50
    if (currentScore <= -50) {
      await supabase.from("scam_entries").upsert([{
        ...
        added_by:   String(adminUserId)
      }], { onConflict: "channel_id,user_id" }).catch(() => {});
    }

    return { approved: true, score: currentScore };
  },
  ...
  /**
   * Setzt einen User auf die Scamliste eines Channels.
   * Idempotent via UNIQUE(channel_id, user_id).
   */
  async addToScamlist(channelId, { userId, username, firstName, reason, adminUserId }) {
    if (!channelId) throw new Error("channelId fehlt");
    if (!userId && !username) throw new Error("userId oder username erforderlich");
    const row = {
      channel_id: String(channelId),
      user_id:    userId ? parseInt(userId) : null,
      username:   username || null,
      first_name: firstName || null,
      reason:     reason || "Auf Scamliste gesetzt",
      added_by:   adminUserId ? String(adminUserId) : null
    };
    const onConflict = userId ? "channel_id,user_id" : undefined;
    const q = supabase.from("scam_entries").upsert([row], onConflict ? { onConflict } : {});
    const { error } = await q;
  ...
  /**
   * Setzt einen User auf die Safeliste eines Channels.
   */
  async addToSafelist(channelId, { userId, username, firstName, reason, adminUserId }) {
    if (!channelId) throw new Error("channelId fehlt");
    if (!userId && !username) throw new Error("userId oder username erforderlich");
    const row = {
      channel_id: String(channelId),
      ...
      username:   username || null,
      first_name: firstName || null,
      reason:     reason || "Auf Safeliste gesetzt",
      added_by:   adminUserId ? String(adminUserId) : null
    };
    const onConflict = userId ? "channel_id,user_id" : undefined;
    const q = supabase.from("channel_safelist").upsert([row], onConflict ? { onConflict } : {});
    const { error } = await q;
    if (error) throw error;
    return row;
  },
