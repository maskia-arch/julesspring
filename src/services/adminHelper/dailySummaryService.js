...
const { t, getLangInstruction } = require("../i18n");

async function createDailySummary(supabase_db, channelId, lang = "de") {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) {
    logger.warn?.("[DailySummary] OPENAI_API_KEY nicht gesetzt");
    return null;
  }
  ...
  try {
    const resp = await axios.post(
      "https://api.openai.com/v1/chat/completions",
      {
        ...
        temperature: 0.3,
        messages: [
          { role: "system", content: getLangInstruction(lang) + "Du bist ein Assistent der Telegram-Gruppen-Tagesberichte erstellt. Fasse die Aktivitäten in 5-8 prägnanten Stichpunkten zusammen. Zensiere Beleidigungen mit [***]. Verwende keine echten Usernamen." },
          { role: "user",   content: `Tagesbericht für diesen Channel:\n\nNachrichten (letzte 24h):\n${lines}\n\nMitglieder: ${joins} aktiv, ${leaves} ausgetreten.` }
        ]
      },
    ...
    logger.warn(`[DailySummary] OpenAI Fehler: ${e.message}`);
    return null;
  }
}

async function runDailySummary(supabase_db, channelId, adminUserId, tg, ch, lang) {
  const result = await createDailySummary(supabase_db, channelId, lang || ch?.bot_language || "de");
  if (!result) {
  ...
  try {
    ...
    creditLogService.log(supabase_db, channelId, {
      category:   "daily_summary",
      credits:    billedTokens,
      tokens_in:  result.inTokens  || 0,
      tokens_out: result.outTokens || 0,
      usd:        actualUsd,
      model:      "gpt-4o-mini"
    });
  } catch (_) {}
