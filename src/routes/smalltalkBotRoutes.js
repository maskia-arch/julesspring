...
async function handleSettingsCallback(tg, supabase_db, data, q, userId) {
  ...
  switch (action) {
    ...
    case "ai": {
      ...
      await tg.call("sendMessage", {
        ...
        reply_markup: { inline_keyboard: [
          ...
          [{ text: "📊 Verbrauch & Limits",  callback_data: `cfg_ai_stats_${channelId}` }]
        ]}
      });
      break;
    }
    ...
    case "ai_stats": {
      const spent = ch?.usd_spent || 0;
      const tUsed = ch?.token_used || 0;
      const tLimit = ch?.token_limit;
      const exhausted = ch?.token_budget_exhausted || false;
      await tg.call("sendMessage", { chat_id: String(userId),
        text: `📊 <b>KI-Verbrauch</b>\n\n` +
              `Token verbraucht: ${tUsed.toLocaleString()}${tLimit ? " / " + tLimit.toLocaleString() : ""}\n` +
              `Kosten: $${spent.toFixed(5)}\n` +
    ...
    case "ai_summary": {
      ...
      if (hoursSince < 24) {
        ...
        await tg.call("sendMessage", { chat_id: String(userId),
          text: `⏳ Tageszusammenfassung nur 1x pro 24h. Nächste möglich um ${nextAt}.` });
        break;
      }
      await tg.call("sendMessage", { chat_id: String(userId), text: "⏳ Erstelle Tageszusammenfassung…" });
      const summary = await _createDailySummary(supabase_db, channelId, tg, String(userId));
      if (summary) {
        await supabase_db.from("bot_channels").update({ last_summary_at: new Date() }).eq("id", channelId).catch(() => {});
        await tg.call("sendMessage", { chat_id: String(userId), text: summary, parse_mode: "HTML" });
      } else {
        await tg.call("sendMessage", { chat_id: String(userId), text: "❌ Fehler bei der Zusammenfassung. Bitte später erneut versuchen." });
      }
      break;
    }
  }
